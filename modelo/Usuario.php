...
class Usuario{
    ...
    function listarestudiante($nombre, $estado){
        $sql = "SELECT u.*,p.nombre perfil FROM usuario u 
                LEFT JOIN perfil p ON u.idperfil=p.idperfil
                WHERE u.idperfil=3 AND u.estado<2 AND u.nombre LIKE :nombre ";
        $parametros = array(':nombre'=>$nombre);
        if($estado!=""){
    ...
    function insertar($dni, $nombre, $correo, $usuario, $clave, $idperfil, $estado){
        $sql = "INSERT INTO usuario(idusuario, dni, nombre, correo, usuario, clave, idperfil, estado) VALUES 
                                   (NULL,:dni,:nombre,:correo,:usuario, SHA1(:clave),:idperfil, :estado)";
        global $cnx;
    ...
    function actualizar($idusuario, $dni, $nombre, $correo, $usuario, $clave, $idperfil, $estado){
        $sql = "UPDATE usuario 
                SET dni=:dni,
                    nombre=:nombre,
                    correo=:correo,
